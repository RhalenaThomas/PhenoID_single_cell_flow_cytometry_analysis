...
For Table S4 look at the predictions within the final cells types

```{r}

# Seurat Label transfer
t.lables <- as.data.frame(table(seu$cell.types, seu$predicted.id))
t.lables$Freq <- as.double(t.lables$Freq)

top.labs.slt <- t.lables  %>% group_by(Var1)  %>% top_n(5, Freq)

# Random Forest
t.lables <- as.data.frame(table(seu$cell.types, seu$rf.pred))
t.lables$Freq <- as.double(t.lables$Freq)

top.labs.rf <- t.lables  %>% group_by(Var1)  %>% top_n(5, Freq)

# Correlation
t.lables <- as.data.frame(table(seu$cell.types, seu$cor.labels))
t.lables$Freq <- as.double(t.lables$Freq)

top.labs.cor <- t.lables  %>% group_by(Var1)  %>% top_n(5, Freq)
...
png(paste(output_path,"UMAP.split.age.12072022.png"),width = 1500, height = 400)
...
# batch 
png(paste(output_path,"UMAP.split.batch.12072022.png"),width = 1150, height = 400)
...
png(paste(output_path,"UMAP.split.exdate.12072022.png"),width = 1150, height = 400)
...
# genotype
sample.lables <- as.data.frame(table(seu$age, seu$cell.types))
sample.lables$Freq <- as.double(sample.lables$Freq)
...
age.order <- c("273","263","284")


df <- sample.lables %>% mutate(Var2 = factor(Var2, levels = cell.type.order))
df <- df %>% mutate(Var1 = factor(Var1, levels = age.order))



pdf(paste(output_path,"bar.pr.age.pdf"), width = 5, height = 3)
ggplot(df, aes(x = Var1,y=Freq ,fill = Var2)) + geom_bar(position= "fill", stat = "identity")  +
  RotatedAxis() +
  scale_y_continuous(labels = scales::percent_format()) + theme_classic() + theme(text = element_text(size=15),
        axis.text.x = element_text(angle=90, hjust=1))  + xlab('Age') + ylab('Percent of Cell type')  + scale_fill_manual(values = clust.colours)
dev.off()
...
# batch
sample.lables <- as.data.frame(table(seu$mo.batch, seu$cell.types))
sample.lables$Freq <- as.double(sample.lables$Freq)
...
df <- sample.lables %>% mutate(Var2 = factor(Var2, levels = cell.type.order))
df <- df %>% mutate(Var1 = factor(Var1, levels = batch.order))



pdf(paste(output_path,"bar.pr.batch.pdf"), width = 5, height = 3)
ggplot(df, aes(x = Var1,y=Freq ,fill = Var2)) + geom_bar(position= "fill", stat = "identity")  +
  RotatedAxis() +
  scale_y_continuous(labels = scales::percent_format()) + theme_classic() + theme(text = element_text(size=15),
        axis.text.x = element_text(angle=0, hjust=1))  + xlab('Batch') + ylab('Percent of Cell type')  + scale_fill_manual(values = clust.colours)
dev.off()
...
sample.lables <- as.data.frame(table(seu$Ex.date, seu$cell.types))
sample.lables$Freq <- as.double(sample.lables$Freq)
...
ex.order <- c("0306","0317")


df <- sample.lables %>% mutate(Var2 = factor(Var2, levels = cell.type.order))
df <- df %>% mutate(Var1 = factor(Var1, levels = ex.order))



pdf(paste(output_path,"bar.pr.exdate.pdf"), width = 5, height = 3)
ggplot(df, aes(x = Var1,y=Freq ,fill = Var2)) + geom_bar(position= "fill", stat = "identity")  +
  RotatedAxis() +
  scale_y_continuous(labels = scales::percent_format()) + theme_classic() + theme(text = element_text(size=15),
        axis.text.x = element_text(angle=90, hjust=1))  + xlab('Experiment Date') + ylab('Percent of Cell type')  + scale_fill_manual(values = clust.colours)
dev.off()

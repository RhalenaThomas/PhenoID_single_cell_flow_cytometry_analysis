...
Read in the gated populations made in FlowJo from the hyper gates
...
Annotate the dataset of FlowJo gated files from hypergate
...
Populations sorted after the FACS gating the some cell acquired
...
input_path <- "/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/FlowDataFiles/PostSortingMay10June10/20220510_sorting_AIW_phenoID/2022-05-10 (S) RT/FCS and experiment files/Posted_Gating_May10"
...
# see how the channels are named
head(flowset@frames$`2022-05-10_post_sort_tris_glia1.fcs`)

# this had to be changed because the channel names were different
flowset <- fsApply(flowset,function(x){x[ ,grepl("^[{'FITC'}|{'FCS'}|{'SSC'}|{'PerCP'}|{'PE'}|{'APC'}|{'BV'}].*A$",colnames(flowset))]}) #Apply a function to flowSet (fsApply) that selects only the columns corresponding to areas values using "regular expression".
...
```{r}
sampleNames(flowset) <- c("Astrocytes","RadialGlia","Neurons1","Neurons2") #directly modify the names inside the flowset
sampleNames(flowset)
...
df.may10 <- read.csv("/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/Analysis/PaperFigures/PostSortGatingPops/may10.csv")
...
seu.may$GatedPop <- factor(x= seu$GatedPop, levels = my_levels)


RidgePlot(seu.may, features = AB, cols = c("orange","pink","mediumpurple2","darkorchid2"), 
...
Repeat to read in the same data with the June 10th post sort cells


```{r}

input_June10 <- "/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/FlowDataFiles/PostSortingMay10June10/20220510_sorting_AIW_phenoID/2022-06-10 (S) RT/FCS and experiment files/Post_Gating_June10"
...
The csv file will have the R object name

```{r}

June10 <- flowset

flowset_to_csv(June10)#apply the function

```

```{r}

df.June10 <- read.csv("/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/Analysis/PaperFigures/PostSortGatingPops/June10.csv")

df2 <- df.June10 %>% dplyr::select(c("CD24","CD56","CD29","CD15","CD184","CD133","CD71","CD44","GLAST","AQP4","HepaCAM", "CD140a","O4"))
m <- as.matrix(df2)
# create the seurat object for visualization
tm <- t(df2)
rownames(tm) <- colnames(df2)
...
seu.june <- ScaleData(seu)

```


See the June 10th data

```{r}


my_levels <- c("Astrocytes","RadialGlia", "Neurons1","Neurons2")
seu.june$GatedPop <- factor(x= seu.june$GatedPop, levels = my_levels)


RidgePlot(seu.june, features = AB, cols = c("orange","pink","mediumpurple2","darkorchid2"), 
          group.by = 'GatedPop',ncol = 4, log = TRUE, slot = 'scale.data')

```

Save each of the seurat objects - Post sorting to compare with the scRNAseq.

```{r}
Idents(seu.may) <- 'orig.ident'
seu.may$orig.ident <- 'May10PostSort'
output_path = "/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/Analysis/PaperFigures/PostSortGatingPops/"

saveRDS(seu.may, paste(output_path,"may10postSort.seu.RDS", sep = ""))

Idents(seu.june) <- 'orig.ident'
seu.june$orig.ident <- 'June10PostSort'

saveRDS(seu.june, paste(output_path,"june10postSort.seu.RDS", sep = ""))
...
Read in post sort FC data from June 10th subset of the scRNAseq sample

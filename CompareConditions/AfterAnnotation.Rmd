...
dotplot of mean expression per group
...
# maybe the different groups need to be made first.
...
df.prp$Batch <- c("B","B","A","B","B","A","B","B","A")
df.prp$Age <- c("273","284","263","273","284","263","273","284","263")
...
# Batch
ggplot(long.prp, aes(y = value, x = Batch, fill = variable)) + geom_bar(position = "stack", stat= "identity")+ theme_classic() + theme(text = element_text(size=15),
        axis.text.x = element_text(angle=90, hjust=1))  + xlab('Sample') + ylab('Proportion of cell phenotypes') + RotatedAxis()
# Ex date
ggplot(long.prp, aes(y = value, x = ExDate, fill = variable)) + geom_bar(position = "stack", stat= "identity")+ theme_classic() + theme(text = element_text(size=15),
        axis.text.x = element_text(angle=90, hjust=1))  + xlab('Sample') + ylab('Proportion of cell phenotypes') + RotatedAxis()
# age
ggplot(long.prp, aes(y = value, x = Age, fill = variable)) + geom_bar(position = "stack", stat= "identity")+ theme_classic() + theme(text = element_text(size=15),
        axis.text.x = element_text(angle=90, hjust=1))  + xlab('Sample') + ylab('Proportion of cell phenotypes') + RotatedAxis()
...
To get expression data grouped in different ways I might be able to use table and interaction features.

To do the proportion tests I can make labels :
1. Genotype
2. EX data
3. Batch number
4. Age
...
```{r}
# make a df for each grouping by making annotation in the seu
...
names(cluster.ids) <- levels(seu.q)    # get the levels
seu.q <- RenameIdents(seu.q, cluster.ids) # rename  
seu.q$Genotype <- Idents(seu.q)   # add a new dataslot

DimPlot(seu.q, reduction = "umap", label = FALSE, group.by = 'cell.types', split.by = 'Genotype')


sample.lables <- as.data.frame(table(seu.q$Genotype, seu.q$cell.types))
...
names(cluster.ids) <- levels(seu.q)    # get the levels
seu.q <- RenameIdents(seu.q, cluster.ids) # rename  
seu.q$Batch.n <- Idents(seu.q)   # add a new dataslot

DimPlot(seu.q, reduction = "umap", label = FALSE, group.by = 'Batch.n', split.by = 'Genotype')
DimPlot(seu.q, reduction = "umap", label = FALSE, group.by = 'cell.types', split.by = 'Batch.n')


sample.lables <- as.data.frame(table(seu.q$Batch.n, seu.q$cell.types))
...
names(cluster.ids) <- levels(seu.q)    # get the levels
seu.q <- RenameIdents(seu.q, cluster.ids) # rename  
seu.q$ExDate <- Idents(seu.q)   # add a new dataslot

DimPlot(seu.q, reduction = "umap", label = FALSE, group.by = 'ExDate', split.by = 'Genotype')
DimPlot(seu.q, reduction = "umap", label = FALSE, group.by = 'cell.types', split.by = 'ExDate')


sample.lables <- as.data.frame(table(seu.q$ExDate, seu.q$cell.types))
...
names(cluster.ids) <- levels(seu.q)    # get the levels
seu.q <- RenameIdents(seu.q, cluster.ids) # rename  
seu.q$Age <- Idents(seu.q)   # add a new dataslot

DimPlot(seu.q, reduction = "umap", label = FALSE, group.by = 'Age', split.by = 'Genotype')
DimPlot(seu.q, reduction = "umap", label = FALSE, group.by = 'cell.types', split.by = 'Age')


sample.lables <- as.data.frame(table(seu.q$Age, seu.q$cell.types))
...
Compare expression within each cell type between different groups


```{r}
seu.q <- NormalizeData(seu.q)

#ave.1 <- as.data.frame(AverageExpression(seu.q,return.seurat = FALSE, group.by = 'Genotype'))

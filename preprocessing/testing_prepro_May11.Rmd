...
output_path <- "/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/Analysis/9MBO/prepro_outs2/"
...
```
Inspect this flowset object

```{r}
# from the May 10th files 
flowset@frames$export_MBO_AIW_2_AUG_2021_PBS_livecells.fcs


# here we can see that the channel names are in the slot 'name' and the name of the antibody is in 'desc'
# the FSC-A and SSC-A have no antibody these are the size measurements 
flowset.may10 <- flowset
flowset.may10@frames$export_MBO_AIW_2_AUG_2021_TRIS_livecells.fcs

```

If I look at the flowset object that works what does it look like?

```{r}
flowset@frames$`2020-03-06- export_bioinfo_AIW_live cells.fcs`
# in this one the description is FSC-A and SSC-A 
...
```{r}
#pdf(paste(output_path,"density_plots.pdf"))
plotdensity_flowset(aligned_transformed_flowset) # error running the alignment
plotdensity_flowset(transformed_flowset) # the transform runs
#dev.off()
...
```{r}
# saving function
flowset = flowset.may10 

  list_of_flowframes=fsApply(rename_markers(flowset),function(x){as.data.frame(x@exprs)})#Makes a list of dataframes
  list_names=names(list_of_flowframes)#extract names for not calling names() function at each loop
  
  # the error is here but it might not be because of the column names NA 
  # the problem seems to be in adding the samples 
  for (index in seq_along(list_of_flowframes)){ #Iterates along the index for adding sample names
    list_of_flowframes[[index]]=list_of_flowframes[[index]] %>% #Using tidyverse package for adding features (name of batch)
      mutate(Sample=list_names[index])#Using tidyverse package for adding features (name of sample/file name)
  #rename the columns to fix the rbind error 
      colnames(list_of_flowframes[[index]]) = colnames(list_of_flowframes[[1]])  
  }
  # this is wehre the error occurs but all the df have the same column names???
